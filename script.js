...
document.getElementById('save-pdf-btn').onclick = async () => {
    ...
    toggleLoader(true, "Carma: Saving PDF...");
...
fileSplit.onchange = async (e) => {
    ...
    if(!file) return;
    splitterPages = [];
    toggleLoader(true, "Carma: Opening Large PDF...");
    ...
    // Show the delete button too
    if(document.getElementById('delete-selected-split-btn')) document.getElementById('delete-selected-split-btn').style.display = 'inline-block';
};
...
// NEW: DELETE SELECTED LOGIC FOR SPLITTER
document.getElementById('delete-selected-split-btn').onclick = () => {
    splitterPages = splitterPages.filter(p => !p.selected);
    if (!splitterPages.length) {
        location.reload(); 
    } else {
        renderSplitterGrid();
    }
...
document.getElementById('split-selected-btn').onclick = async () => {
    ...
    toggleLoader(true, "Carma: Splitting Spread Pages...");
    try {
...
document.getElementById('file-conv').onchange = async (e) => {
    ...
    toggleLoader(true, "Carma: Batch Exporting...");
